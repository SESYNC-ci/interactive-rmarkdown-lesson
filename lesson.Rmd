...
The handout for this lesson _is_ this lesson. The file ``r handouts[[1]]`` is
...
The ``r handouts[[2]]`` file is an R script that produces a `rodents` data
...
```{r load_data, echo = FALSE, cache.extra = md5sum(params$handouts[2])}
source(params$handouts[2])
```
...
```
```{r load_data, echo = FALSE, cache.extra = md5sum('`r handouts[2]`')}
source('`r params$handouts[2]`')
```
```

<div class="notes">
A change to the `rodents` variable, possibly resulting from filtering in a
different year in the ``r handouts[2]``, will now be reflected in the
...
See an example at `r paste0('https://', params$github_account, '.github.io/handouts/')`.
